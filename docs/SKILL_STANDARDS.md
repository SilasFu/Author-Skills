...
## 6. 复合工程化套件规范 (Composite Tool-Augmented Structure)

工业级自研 Skill 推荐采用复合工程化解耦结构：
- **`SKILL.md`**：轻量控制中枢与 SOP 调度（严格 ≤ 300 行）；
- **`scripts/`**：确定性自动化执行脚本（Python 3，承载所有行数扫描、正则匹配、编译自检等机械计算，输出结构化 JSON/卡片）；
- **`templates/`**：金标契约模板（参数化插槽，消除大模型每次手写生成时的格式漂移）；
- **`references/`**：领域手册与详细分类字典（渐进披露，按需深读）。
