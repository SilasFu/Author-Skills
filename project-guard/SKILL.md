...
| 模式 | 用户典型触发词 | 核心动作 |
| :--- | :--- | :--- |
| **init (初始化植入)** | “帮这个项目初始化规范”、“规范化新项目”、“植入质量机制”、“/project-guard init” | 自动检测技术栈，生成项目级 AGENTS.md 与 docs/design/DESIGN.md，固化编译器与防腐门禁。 |
...
### 3. 自动生成前端设计契约 docs/design/DESIGN.md（若存在前端）
若项目包含前端，生成标准设计 Token 文件，固化：
- **中性灰阶基调**：Canvas 背景、Surface 卡片、Border 细线、Foreground 正文、1 种极度克制的主强调色。
- **去卡片化原则**：严禁卡片套卡片（Card inside Card），优先使用留白和单像素细线分割。
- **信息密度**：单屏高密度工作台，拒绝低效巨大留白。

### 4. 验证与交付
- 运行一次类型检查或编译命令，确认当前项目无语法报错。
- 输出初始化报告，向用户展示生成的 AGENTS.md 与 DESIGN.md 路径。

**完成标准**：根目录存在 AGENTS.md，docs/design/DESIGN.md（前端项目）就绪，编译器自检命令验证通过。
